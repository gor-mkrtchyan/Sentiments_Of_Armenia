...
def recommend_from_query(
    ...
    for name in embeddings.keys():
        # ensure we have a row for this name to check city/province
        row = df[df["Location Name"] == name].head(1)
        if row.empty:
            continue

        rest_city = str(row["Town/City"].iloc[0]) if "Town/City" in row.columns else ""
        rest_province = str(row["Province"].iloc[0]) if "Province" in row.columns else ""
